...
- **This Claude Code sandbox cannot reach YouTube or Substack** (`natesnewsletter.substack.com`, including its `/feed` and individual post URLs) — blocked by this environment's network egress proxy, confirmed via direct test. GitHub is reachable.
- **Nate B. Jones** (newsletter, YouTube, GitHub `NateBJones-Projects`) is Eric's trusted source for AI engineering ideas/tools, especially relevant to the still-undesigned security work (memory poisoning / prompt injection defenses, plan doc §8). Since the newsletter can't be fetched directly from here, **paste article text/links when a specific piece is needed.**
  - Known repos of interest (via GitHub, not independently verified beyond the fetch summary): **Ringer** (parallel orchestrator for AI agent swarms), **AI Airlock** (macOS-only document sanitizer for AI tools — not usable for Eric, no macOS available).
  - "Open Brain" — referenced by Eric as something he'd set up before that stopped working; likely covered in a specific Substack article he linked, but that article is also unreachable from here. Still unresolved — revisit when Eric pastes the content or when it becomes actually relevant (not blocking anything right now).
- **Devices:** Eric uses Chrome/Chromebook, the Claude app, and a Windows laptop (no macOS). This session runs in the cloud regardless of what device he's on — no device switch needed unless a specific future tool requires local execution (e.g., Ringer, if it needs a local desktop). Nothing currently requires this.
