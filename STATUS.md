...
- **Abacus.AI skill added:** `.claude/skills/abacus-ai/` — a Claude Code skill for calling Abacus.AI (REST, the `abacusai` Python SDK, RouteLLM) with minimum token/dollar spend: explicit model routing (cheap models for routine work, frontier for ambiguous/governance work), pulling only relevant RAG chunks via `get_matching_documents` instead of whole documents, and static-prefix/dynamic-suffix prompt shape for caching. All API details in it were verified directly against the installed `abacusai` SDK source, not memorized docs. **Confirmed limitation:** this Claude Code cloud sandbox's network egress proxy blocks all `abacus.ai` domains outright (403 on CONNECT) — so a live connection couldn't be tested from this session even with a real API key. That's an environment network-policy fact, not a credentials problem, and won't apply everywhere (e.g. a local machine, or a differently-configured environment).
